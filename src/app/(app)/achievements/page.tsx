...
import { Award, Lock, Diamond, Bot, TrendingUp, Trophy, Zap, ShieldQuestion } from "lucide-react";
...
export default function AchievementsPage() {
  return (
    <div className="container py-8">
      ...
      <p className="text-muted-foreground mb-8 text-xl font-body">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {achievementsData.map((achievement) => {
            ...
            return (
          <Card key={achievement.id} className={cn("flex flex-col border-2", isLocked ? "border-muted/50 bg-muted/20 text-muted-foreground" : "border-secondary")}>
            <CardHeader className="flex-row gap-4 items-center">
              ...
              <div>
                ...
                <CardDescription className={cn("font-body text-base", isLocked ? "text-muted-foreground/70" : "text-accent")}>Reward: {achievement.reward} DREAM</CardDescription>
              </div>
            </CardHeader>
            <CardContent className="flex-1 flex flex-col justify-between gap-4">
              <p className="text-lg font-body">{achievement.description}</p>
              <div>
                {achievement.status === 'unlocked' && (
                  <div className="text-lg font-semibold text-primary font-headline">COMPLETED</div>
                )}
